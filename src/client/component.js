...
export default class SPAMain extends React.Component {
  ...
  render() {
    // TODO views.border
    return <>
      {createElement(components.pages[this.state.renderPage], {
        ...this.state.pages[this.state.renderPage],
        ...this.state.data,
        ...controllerStreamsMapped.pages[this.state.renderPage]
      })}
      {Object.keys(components.views).filter(n => n !== 'border').map(key => createElement(components.views[key], {
        ...this.state.views[key],
        ...this.state.data,
        ...controllerStreamsMapped.views[key]
      }))}
      {Object.keys(this.state.models).map(type => Object.keys(this.state.models[type]).map(id => createElement(components.models[type], {
